    ...
    <footer className="w-full bg-[#E3EAF7] text-veto-black min-h-[400px] lg:h-[400px] px-8 md:px-16 pt-16 pb-8 border-t-[12px] border-veto-yellow transition-colors flex flex-col justify-between relative overflow-hidden">
      ...
      <div className="max-w-[1400px] mx-auto w-full grid grid-cols-1 md:grid-cols-12 gap-8 relative z-10 flex-1 content-center">
        ...
        <div className="md:col-span-3 flex flex-col items-center md:items-start gap-6 mt-4 md:mt-0 text-center md:text-left">
          <h4 className="font-black text-lg tracking-widest uppercase text-veto-black">{t('footer.menu')}</h4>
          <ul className="flex flex-col gap-3 font-bold text-sm text-veto-gray">
            <li><a href="/#services" className="hover:text-veto-black transition-colors">{t('nav.services')}</a></li>
            <li><a href="/about" className="hover:text-veto-black transition-colors">{t('footer.about')}</a></li>
            <li><a href="#contact" className="hover:text-veto-black transition-colors">{t('footer.contact')}</a></li>
          </ul>
        </div>
      ...
      <div className="max-w-[1400px] w-full mx-auto pt-6 border-t border-black/10 flex flex-col md:flex-row items-center justify-between gap-4 text-[9px] md:text-[10px] font-bold uppercase tracking-[0.2em] text-veto-gray relative z-10 mt-8 text-center md:text-left">
        <div className="flex flex-col sm:flex-row gap-1 sm:gap-0 items-center">
          ...
          <span className="text-veto-black">{t('footer.rights')}</span>
        </div>
        <ul className="flex flex-wrap justify-center gap-4 sm:gap-8">
          <li><a href="#" className="hover:text-veto-black transition-colors">{t('footer.privacy')}</a></li>
          <li><a href="#" className="hover:text-veto-black transition-colors">{t('footer.terms')}</a></li>
        </ul>
        <button
          onClick={scrollToTop}
