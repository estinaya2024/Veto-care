...
export function Services() {
  ...
  const services = [
    { key: 'services.vaccinations', icon: Syringe },
    { key: 'services.medical_record', icon: FileText },
    { key: 'services.vet_visits', icon: Stethoscope },
    { key: 'services.prescriptions', icon: ClipboardList },
    { key: 'services.appointments', icon: Calendar },
    { key: 'services.pet_profile', icon: PawPrint },
  ];
  ...
  return (
    <section id="services" className="px-6 md:px-16 py-16 md:py-24 bg-white text-center relative overflow-hidden">
      ...
      <div className="relative inline-block mb-12 md:mb-16">
        <Heading level={2} className="text-4xl md:text-5xl lg:text-6xl">{t('services.heading')}</Heading>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-6 md:gap-8 max-w-6xl mx-auto relative z-20">
        {services.map((service, index) => (
          <div key={index} className="flex flex-col items-center gap-4 group cursor-pointer">
            ...
            <span className="font-bold text-veto-black tracking-tight text-sm md:text-base">{t(service.key)}</span>
          </div>
        ))}
      </div>
    </section>
